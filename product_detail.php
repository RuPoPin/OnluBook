...
if (isset($_GET['id']) && filter_var($_GET['id'], FILTER_VALIDATE_INT)) {
    ...
    if ($connect) {
        ...
        error_log("Database connection not established for product loading.");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
    ...
    if (!isset($_SESSION['user']['id']) || !isset($_SESSION['user']['name'])) { 
        ...
        $redirect_product_id_from_form = filter_var($_POST['product_id'] ?? null, FILTER_VALIDATE_INT);
        $current_product_id_for_redirect = $redirect_product_id_from_form ?: $product_id;

        if ($current_product_id_for_redirect) {
            $redirect_url = strtok($_SERVER["REQUEST_URI"], '?') . "?id=" . $current_product_id_for_redirect . "#review-form-section";
            header("Location: " . $redirect_url);
            exit();
        } else {

            $fallback_redirect_url = $_SERVER['REQUEST_URI'];
            if (strpos($fallback_redirect_url, '#') === false) {
                $fallback_redirect_url .= "#review-form-section";
            } else {
                $fallback_redirect_url = preg_replace('/#.*$/', '', $fallback_redirect_url) . "#review-form-section";
            }
    ...
    $text = trim(strip_tags($text_raw)); 
    
    $user_id = (int)$_SESSION['user']['id']; 


    if (!$review_product_id || ($product && $review_product_id !== $product['id'])) {
        $review_error_message = "Ошибка: неверный товар для отзыва. ID из формы: {$review_product_id}, ID товара страницы: " . ($product['id'] ?? 'N/A');
    } elseif ($rating === false) {
        ...
        $review_error_message = "Текст отзыва слишком короткий (минимум 10 символов). Текущая длина: " . mb_strlen($text);
    } else {
        if ($connect) {
            $is_approved = 1; 
            ...
            if ($stmt_insert_review) {
                ...
                if ($stmt_insert_review->execute()) {
                    ...
                    error_log("Failed to execute insert review query: " . $stmt_insert_review->error);
                    error_log("SQL Error (insert review): " . $stmt_insert_review->error);
                    error_log("Review Data for Insert: product_id=$review_product_id, user_id=$user_id, author_name=$author_name, rating=$rating, text_length=" . mb_strlen($text) . ", is_approved=$is_approved");

                    $review_error_message = "Не удалось отправить отзыв. Ошибка базы данных при выполнении.";
                }
                $stmt_insert_review->close();
            } else {
                error_log("Failed to prepare insert review query: " . $connect->error);
                $review_error_message = "Ошибка сервера при подготовке запроса для сохранения отзыва.";
            }
        } else {
            error_log("Database connection not established for review submission.");
            $review_error_message = "Ошибка подключения к базе данных при отправке отзыва.";
        }
    }
...
// Получение flash-сообщений из сессии
...
<head>
    ...
    <style>
        ...
        .rating-stars-input { display: inline-flex; direction: rtl; /* звезды справа налево */ }
        .rating-stars-input input[type="radio"] { display: none; }
        .rating-stars-input label { font-size: 2em; color: #ddd; cursor: pointer; padding: 0 0.1em; }
        .rating-stars-input input[type="radio"]:checked ~ label,
        .rating-stars-input label:hover,
        .rating-stars-input label:hover ~ label { color: #f5a623; }
    </style>
</head>
<body>
    <?php 
...
<main class="product-detail-page modern-minimalist">
    <div class="container">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="rating-stars-input">
                                        <?php for ($r_i = 5; $r_i >= 1; $r_i--): 
                                            ...
                                            } elseif (!isset($form_data['rating']) && $r_i == 0) { 
                                            }
                                        ?>
        ...
        <?php elseif ($error_message): ?>
            <div class="error-message alert alert-danger" style="text-align:center; padding: 20px;">
                <p><?= htmlspecialchars($error_message); ?></p>
                <p><a href="index.php" class="btn-cta" style="text-decoration:none; display:inline-block; margin-top:10px;">Вернуться на главную</a></p>
            </div>
        <?php else:?>
             <div class="error-message alert alert-danger" style="text-align:center; padding: 20px;">
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.quantity-selector').forEach(selector => {
        ...
        btnMinus.addEventListener('click', () => { if (parseInt(input.value) > minQuantity) input.value = parseInt(input.value) - 1; });
        btnPlus.addEventListener('click', () => { if (parseInt(input.value) < maxQuantity) input.value = parseInt(input.value) + 1; });
        if (input.readOnly) {
             input.addEventListener('keydown', (e) => {
                if (!(e.key === 'ArrowLeft' || e.key === 'ArrowRight' || e.key === 'Tab' || e.key === 'Backspace' || e.key === 'Delete')) {
                    e.preventDefault();
                }
            });
        }
    ...
    document.querySelectorAll('.wishlist-btn:not(.disabled)').forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (this.classList.contains('active')) {
                if(icon) icon.classList.replace('fa-heart-o', 'fa-heart'); 
                if(textSpan) textSpan.textContent = 'В избранном';
            } else {
                if(icon) icon.classList.replace('fa-heart', 'fa-heart-o'); 
    ...
    if (serviceInfoBlock && overlay) { 
        ...
        if (!serviceInfoBlock) console.warn("Service info block ('.product-service-info') not found, modals might not be triggerable from there.");
    }
        ...
        if (reviewAlert) {
            ...
            if (isAlertVisible) { 
                if (reviewsTabContent && !reviewsTabContent.classList.contains('active')) {
                     openTabAndScroll(null, 'reviews-section', reviewAlert.id || (reviewAlert.closest('[id]') ? reviewAlert.closest('[id]').id : 'review-form-section'));
